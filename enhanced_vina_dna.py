#!/usr/bin/env python3
"""
Enhanced Vina-like Scoring Functions for Protein-DNA Complexes (Standalone)
...
class AtomProperties:
    ...
    DNA_CHARGES = {
        'DA': {  # Adenine
            'P': 1.1659, 'O1P': -0.7761, 'O2P': -0.7761, 'O5\'': -0.4954,
            'C5\'': 0.0558, 'C4\'': 0.1065, 'O4\'': -0.3548, 'C1\'': 0.0394,
            'N9': -0.0268, 'C8': 0.1607, 'N7': -0.6175, 'C5': 0.0725,
            'C6': 0.6897, 'N6': -0.9123, 'N1': -0.7624, 'C2': 0.5716,
            'N3': -0.7417, 'C4': 0.3800, 'C2\'': -0.0854, 'C3\'': 0.0713,
            'O3\'': -0.6549
        },
        'DT': {  # Thymine
            'P': 1.1659, 'O1P': -0.7761, 'O2P': -0.7761, 'O5\'': -0.4954,
            'C5\'': 0.0558, 'C4\'': 0.1065, 'O4\'': -0.3548, 'C1\'': 0.0680,
            'N1': -0.0239, 'C6': -0.2209, 'C2': 0.5194, 'O2': -0.4293,
            'N3': -0.4340, 'C4': 0.5952, 'O4': -0.5761, 'C5': -0.1626,
            'C7': -0.2269, 'C2\'': -0.0854, 'C3\'': 0.0713, 'O3\'': -0.6549
        },
        'DG': {  # Guanine
            'P': 1.1659, 'O1P': -0.7761, 'O2P': -0.7761, 'O5\'': -0.4954,
            'C5\'': 0.0558, 'C4\'': 0.1065, 'O4\'': -0.3548, 'C1\'': 0.0191,
            'N9': 0.0492, 'C8': 0.1374, 'N7': -0.5725, 'C5': 0.1991,
            'C6': 0.4918, 'O6': -0.5699, 'N1': -0.5053, 'C2': 0.7432,
            'N2': -0.9230, 'N3': -0.6636, 'C4': 0.1814, 'C2\'': -0.0854,
            'C3\'': 0.0713, 'O3\'': -0.6549
        },
        'DC': {  # Cytosine
            'P': 1.1659, 'O1P': -0.7761, 'O2P': -0.7761, 'O5\'': -0.4954,
            'C5\'': 0.0558, 'C4\'': 0.1065, 'O4\'': -0.3548, 'C1\'': 0.0066,
            'N1': -0.0484, 'C6': -0.0183, 'C2': 0.5952, 'O2': -0.6548,
            'N3': -0.7584, 'C4': 0.8185, 'N4': -0.9530, 'C5': -0.5215,
            'C2\'': -0.0854, 'C3\'': 0.0713, 'O3\'': -0.6549
        }
    }
...
class DNAGeometry:
    """DNA geometry and groove analysis utilities"""
    
    @staticmethod
    def is_in_major_groove(protein_atom: Atom, dna_residue: Residue) -> bool:
        """Check if protein atom is positioned in DNA major groove"""
        protein_coord = protein_atom.get_coord()
        
        # Get DNA base atoms for groove identification
        base_atoms = []
        ...
        # Calculate approximate groove position
        base_center = np.mean([atom.get_coord() for atom in base_atoms], axis=0)
        distance = np.linalg.norm(protein_coord - base_center)
        
        # Major groove interactions are typically at intermediate distances
        return 4.0 <= distance <= 8.0
    
    @staticmethod
    def is_in_minor_groove(protein_atom: Atom, dna_residue: Residue) -> bool:
        """Check if protein atom is positioned in DNA minor groove"""
        protein_coord = protein_atom.get_coord()
        
        # Get sugar atoms for minor groove identification
        sugar_atoms = []
        for atom in dna_residue:
        ...
        # Minor groove interactions are typically closer to sugar backbone
        return 3.0 <= distance <= 6.0


class EnhancedVinaScorer:
    """Enhanced Vina-like scoring function with DNA-specific improvements"""
    ...
    def __init__(self, 
        ...
        # Enhanced weights for protein-DNA interactions
        self.weights = {
            'gauss1': -0.0356,
            'gauss2': -0.00516,
            'repulsion': 0.840,
        ...
        self.major_groove_bonus = 1.5
        self.minor_groove_bonus = 1.2
        self.phosphate_cutoff = 5.0
    
    def load_structure(self, structure_file: str) -> Optional[Structure]:
        """Load structure from PDB/CIF file"""
        ...
        try:
            ...
            return parser.get_structure('complex', str(structure_file))
    ...
    def identify_chains(self, structure: Structure) -> Dict[str, List[str]]:
        ...
        for model in structure:
            for chain in model:
                residue_names = [res.get_resname() for res in chain if res.id[0] == ' ']
                ...
                if total > 0 and dna_count / total > 0.5:
    ...
    def calculate_electrostatic_term(self, atom1: Atom, atom2: Atom,
        ...
        """Calculate electrostatic interaction with AMBER charges"""
    ...
    def calculate_dna_specific_terms(self, protein_atom: Atom, dna_atom: Atom,
        ...
        if DNAGeometry.is_in_major_groove(protein_atom, dna_residue):
            terms['major_groove'] = self.major_groove_bonus * np.exp(-surface_distance**2 / 4.0)
        else:
            terms['major_groove'] = 0.0
        
        # Minor groove interactions
        if DNAGeometry.is_in_minor_groove(protein_atom, dna_residue):
            terms['minor_groove'] = self.minor_groove_bonus * np.exp(-surface_distance**2 / 4.0)
        else:
        ...
        if dna_atom.get_name() in ['P', 'O1P', 'O2P', 'O5\'', 'O3\'']:
            ...
            if distance < self.phosphate_cutoff:
    ...
    def score_complex(self, structure_file: str) -> Dict[str, float]:
        """Score protein-DNA complex with enhanced terms"""
        ...
        if not protein_chains or not dna_chains:
            print("Warning: Could not identify both protein and DNA chains")
            return {}
                ...
                if chain.id in protein_chains:
                    for residue in chain:
                        if residue.id[0] == ' ':  # Standard residues only
                            for atom in residue:
                                if atom.element != 'H':
                                    protein_atoms.append((atom, residue))
                
                elif chain.id in dna_chains:
                    for residue in chain:
                        if residue.id[0] == ' ':
                            for atom in residue:
                                if atom.element != 'H':
                                    dna_atoms.append((atom, residue))
        
        print(f"Analyzing {len(protein_atoms)} protein atoms and {len(dna_atoms)} DNA atoms")
        ...
        # Calculate all protein-DNA interactions
        for protein_atom, protein_residue in protein_atoms:
            for dna_atom, dna_residue in dna_atoms:
                ...
                vdw1 = AtomProperties.get_vdw_radius(protein_atom.element)
                vdw2 = AtomProperties.get_vdw_radius(dna_atom.element)
                surface_distance = center_distance - vdw1 - vdw2
                ...
                total_scores['hydrophobic'] += self.calculate_hydrophobic_term(
                    ...
                    protein_residue.get_resname(), dna_residue.get_resname())
                ...
                total_scores['electrostatic'] += self.calculate_electrostatic_term(
                    protein_atom, dna_atom, protein_residue.get_resname(),
                    dna_residue.get_resname(), center_distance)
    ...
    def print_detailed_results(self, results: Dict[str, float]):
        ...
        print("\n" + "="*60)
        print("ENHANCED VINA-LIKE SCORING RESULTS")
        print("="*60)
        ...
        print(f"Interactions Analyzed: {results['interaction_count']}")
        ...
        for term in energy_terms + dna_terms:
...
def main():
    ...
    if len(sys.argv) != 2:
        print("Usage: python enhanced_vina_dna_standalone.py <structure_file>")
        print("\nEnhanced Vina-like Scoring for Protein-DNA Complexes")
        print("Features:")
        print("  - AMBER-based partial charges")
        print("  - DNA groove-specific interactions")
        print("  - Phosphate backbone recognition")
        print("  - Base stacking interactions")
